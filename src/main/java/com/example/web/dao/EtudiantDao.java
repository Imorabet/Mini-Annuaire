...
public class EtudiantDao {
	...
	public List<Etudiant> getEtudiants() {
		...
		try (Connection connection = DriverManager.getConnection(URL, USERNAME, PASSWORD)) {
			String query = "SELECT e.*, f.*, d.* " + "FROM etudiant e " + "INNER JOIN filiere f ON e.filiere_id = f.id "
	...
	public List<Etudiant> rechercheEtudiant(String studentName) {
		List<Etudiant> etudiants = new ArrayList<>();

		try (Connection connection = DriverManager.getConnection(URL, USERNAME, PASSWORD)) {
			String query = "SELECT e.CNE, e.nom, e.prenom, e.telephone, f.nom AS filiere_nom " + "FROM etudiant e "
					+ "INNER JOIN filiere f ON e.filiere_id = f.id " + "WHERE e.nom = ?";
